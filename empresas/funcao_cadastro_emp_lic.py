...
def associar_empresa_a_tag_ou_licenca():
    try:
        with criar_conexao() as conexao:
            ...
            if opcao == "1":
                atribuir_tag_a_empresa(cnpj_escolhido, empresa[0])

            elif opcao == "2":
                adicionar_licencas_manual(cnpj_escolhido, empresa[0])
...
def atribuir_tag_a_empresa(cnpj, nome_empresa):
    try:
        with criar_conexao() as conexao:
...
def adicionar_licencas_manual(cnpj, nome_empresa):
    try:
        with criar_conexao() as conexao:
            cursor = conexao.cursor()
